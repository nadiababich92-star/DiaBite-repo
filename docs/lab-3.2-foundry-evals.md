# Lab 3.2, adapted for DiaBite — evaluating the agent in Azure AI Foundry

> **Updated 23 September 2026.** The agent now runs in Foundry itself, and the
> dataset is generated by `npm run eval:agent` against the deployed
> `POST /agent/ask` instead of being collected by hand in the app. The
> hand-collection steps below are kept because they are the lab as taught; the
> Foundry phases are unchanged either way.
...
No tunnel is involved any more: the engine and the agent both run in Azure, and the app talks to `POST /agent/ask`. Collecting answers by hand is now optional — `npm run eval:agent` replays every case against the deployed agent and writes the dataset.
...
- **Judge model**: Foundry's model-graded evaluators use a model deployed in
  Azure. It must differ from the model under test — the agent runs on
  `gpt-5-mini`, so the judge is a separate deployment.
